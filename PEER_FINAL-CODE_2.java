...
class peer_
{
	...
	else
	{
		...
		if(c!=0)
		{
		System.out.println("Do you want help from Peer or a Teacher???? Press P for Peer T for Teacher and Anykey for All options");
		opt=br.readLine();
		for(k=0;k<i;k++)
		{
		if(k!=j)
		{
			...
			{
			if((desig[k]).equalsIgnoreCase("Teacher") && (subject[k]).equalsIgnoreCase(subject[j]) && (weekday[k]).equalsIgnoreCase(weekday[j]))
			{
			arr[p++]=k;
			}
			}
			...
			{
			if((desig[k]).equalsIgnoreCase("Student") && (subject[k]).equalsIgnoreCase(subject[j]) && (choice[k]).equalsIgnoreCase("t") && (weekday[k]).equalsIgnoreCase(weekday[j]))
			{
			arr[p++]=k;
			}
			}
		...
		if(p!=0)
		{
		System.out.println("Your Peer(s) or Teacher(s) Found: "+"Designation "+"Name");
		for(n=0;n<p;n++)
		{
		...
		if(opt.equalsIgnoreCase("t") || (desig[arr[ch-1]]).equalsIgnoreCase("Teacher"))
		{
		System.out.println("You Are Scheduled to Meet Prof. "+name[arr[ch-1]]+" on "+weekday[arr[ch-1]]);
		}
		else
		{
